...
\subsubsection{Experimental method}
	...
	\begin{equation}
		\rtDistIter{i}{0} = \extDist{i} \otimes \bigotimes_{j \in hp(i)} \extDist{j}
	\end{equation}
	...
	\begin{definition}[Safety Metric]
		...
		\begin{equation}
			\mathcal{S}(Pr(r_i > D_i ; \boldsymbol{P}) - \Theta_i)
	...
	\begin{equation}
		\textbf{SP}(\boldsymbol{P}; \textbf{E}_k) \triangleq \sum_{\tau_i \in \boldsymbol{\tau}} \
...
\end{figure}
	...
	\end{equation}
	\begin{equation}
		\rtDist{i} = \mathcal{R}(\extDist{i}(\textbf{E}_k;\mathcal{Q}_i), \text{hp}(i))
	...
	\end{equation}
        ...
        In this paper, we adopt fixed-task priority scheduling under the scheduling algorithm $\mathcal{A}$, so the optimization variable $\boldsymbol{P}$ is the priority assignment for each task.
        ...
        $hp(i)$ denotes the tasks with strictly higher priority than $\tau_i$.
