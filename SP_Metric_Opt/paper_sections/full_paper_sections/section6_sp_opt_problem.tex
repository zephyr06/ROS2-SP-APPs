...
\subsubsection{Experimental method}
	...
	\begin{equation}
        ...
        \label{eq: rta_scalar}
	\end{equation}
    where $hp(i)$ denotes the set of tasks with strictly higher priority than $\tau_i$ (a task is not in its own $hp$ set).
	...
	\begin{equation}
		R_i^0 = \extDist{i} \otimes \bigotimes_{j \in hp(i)} \extDist{j}
	\end{equation}
	where $\otimes$ denotes convolution and $hp(i)$ denotes the set of tasks with strictly higher priority than $\tau_i$.
...
\end{figure}
	...
	\end{equation}
        ...
        $\hptasks{i}$ denotes the tasks with strictly higher priority than $\tau_i$.
