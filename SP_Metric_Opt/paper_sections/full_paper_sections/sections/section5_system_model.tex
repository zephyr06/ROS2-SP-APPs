...
QoS budget                                                  & $\mathcal{Q}_i$     \\
Task's deadline miss threshold                                        & $\Theta_i$      \\
Task's importance                                                     & $w_i$           \\
Important task subset                                                  & $\boldsymbol{\tau}^{safe}$           \\
Higher-priority task set of $\tau_i$ (given $\boldsymbol{\tau},\boldsymbol{P},\boldsymbol{\mathcal{Q}}$) & $\text{hp}(i;\boldsymbol{\tau},\boldsymbol{P},\boldsymbol{\mathcal{Q}})$ \\  \midrule
...
$s$-th challenger solution during search                              & $\boldsymbol{P}^{(s)},\boldsymbol{\mathcal{Q}}^{(s)}$ \\
Champion solution while evaluating the $s$-th challenger              & $\boldsymbol{P}^{(s)*},\boldsymbol{\mathcal{Q}}^{(s)*}$ \\
...
\end{table}
    ...
    Each task $\tau_i$ is characterized by five parameters: probability distribution of execution time $\extDist{i}$, period $T_i$, deadline $D_i$, and its Quality-of-Service (QoS) budget $\mathcal{Q}_i$:
	\begin{equation}
		\tau_i: [\extDist{i}, T_i, D_i, \mathcal{Q}_i]
	...
	\begin{definition}[QoS budget, $\mathcal{Q}_i$]
		\label{def_task_config}
		A task $\tau_i$'s Quality-of-Service (QoS) budget $\mathcal{Q}_i$ is the parameter that influences the task's execution time distribution and output quality.
