...
The price BF pays for this near-optimality is steep: its optimization time grows from $0.12$~s
($N{=}4$) to $35$~s ($N{=}6$) and $424$~s ($N{=}8$), while INCR stays at $0.004$--$0.011$~s,
...
\midrule
4 & 0.933 & 0.932 & 0.2\% & 0.120 & 0.0036 & $3.4\times10^{1}$ \\
6 & 0.792 & 0.779 & 1.7\% & 35.22 & 0.0047 & $7.4\times10^{3}$ \\
8 & 0.758 & 0.742 & 2.1\% & 424.5 & 0.0110 & $3.9\times10^{4}$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\columnwidth]{pictures/simulation/simulation_scale/fig1a_mean_sp_normalized_vs_tasks_main.pdf}
...
$\boldsymbol{\tau}^{safe}$, one line per task count $N \in \{4,\dots,16\}$.
INCR holds the rate essentially at zero---below each task's threshold $\Theta_i$---at every $N$, the
direct effect of the in-search important-task feasibility gate (Section~\ref{section_config_opt});
its only nonzero reading is $0.0004$ at $N{=}12$.
The baselines without the gate let important-task misses climb under overload: CFS peaks at $0.145$
($N{=}12$), BF at $0.098$ ($N{=}12$), and DM\_SLOW at $0.044$ ($N{=}14$).
DM\_FAST also stays at zero, but only because its small QoS budgets spare enough utilization for its
fixed priority order to meet important deadlines---at the large SP cost already shown in
Fig.~\ref{fig_sim_sp_baselines}.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\columnwidth]{pictures/simulation/simulation_scale/fig3_important_task_miss_rate.pdf}
    \caption{Important-task deadline-miss rate (one line per task count $N$).
    INCR suppresses important-task misses at every $N$ via the in-search feasibility gate; baselines
    without the gate let them climb under overload.}
...
\begin{figure*}[!htbp]
    ...
    \begin{subfigure}[c]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{pictures/simulation/simulation_scale/fig_ablation_mean_sp_normalized_vs_tasks.pdf}
        \caption{Normalized SP of ablation arms.}
        \label{fig_sim_ablation_sp}
    \end{subfigure}\hfill
    \begin{subfigure}[c]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{pictures/simulation/simulation_scale/fig_ablation_mean_exec_time_vs_tasks.pdf}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\columnwidth]{pictures/simulation/simulation_scale/fig_fallback_rejection_ratio.pdf}
