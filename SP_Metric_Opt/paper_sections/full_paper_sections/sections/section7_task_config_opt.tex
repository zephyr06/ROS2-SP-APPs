    ...
    A step is deemed successful if it strictly improves the system-level SP metric, with ties broken toward the smaller budget so that equal-SP options resolve to the cheaper one.
    ...
    The patience budget bounds the computation spent on non-improving directions, concentrating online effort where it yields SP gains, and its value is selected by optimization mode: a re-optimization interval started from a fresh seed permits a longer search than an incremental one.
