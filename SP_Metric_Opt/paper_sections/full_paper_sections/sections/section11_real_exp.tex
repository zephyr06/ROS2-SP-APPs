...
\end{table}
    ...
    We evaluate five scheduling algorithms, used consistently in both the real-world
    experiments here and the simulation experiments (Section~\ref{section: simulation}):
	\begin{itemize}
		\item CFS (Complete Fair Scheduling). CFS is the default scheduling algorithm in most
        Linux systems. It balances CPU utilization among all running tasks based on their
        importance and aims to achieve fair resource sharing. However, CFS is not a
        real-time scheduling algorithm and cannot provide performance guarantees.
		\item DM\_FAST and DM\_SLOW (Deadline-Monotonic, min/max QoS budget).
        These fixed-priority schedulers order tasks by Deadline-Monotonic (DM) priorities
        (shorter relative deadlines first) and fix each QoS-configurable task's budget to an
        extreme of its grid, as in Definition~\ref{def_dm_fast}:
        DM\_FAST uses the shortest budget, DM\_SLOW the longest.
        Under the implicit-deadline model $D_i = T_i$ used in our experiments, DM coincides
        with Rate-Monotonic (period-based) ordering.
        In the real-world system TSP is the sole QoS-configurable task, so its budget is set
        to the shortest (DM\_FAST) or longest (DM\_SLOW) execution time.
		\item BF (Brute-Force scheduler).
        This scheduler uses the brute-force optimization algorithms introduced in
        Section~\ref{section_bf_pa} and Section~\ref{section_config_opt} as a baseline method.
		\item INCR (Incremental scheduler).
        This scheduler applies the incremental optimization algorithms described in
        Section~\ref{section_increment_pa} and Section~\ref{section_config_opt}, optimizing
        both priority assignments and QoS budgets; it is the proposed method.
	\end{itemize}
...
\subsection{Results}
    ...
    % \todo consider why CFS and DM-Slow miss a few data points at the end
    % In Fig.~\ref{fig_cfs_perf} and \ref{fig_dm_slow}, at the end of experiments (around 830s), some data points are missing due to the high system utilization and uneven distribution of work loads, so some applications
	\begin{figure*}[ht!]
		\centering 
  ...
  %           \includegraphics[width=\textwidth]{pictures/average_all.pdf}
    ...
    TSP's execution time is determined by its runtime parameters and shows stable probability distributions under CFS, DM\_SLOW, and DM\_FAST schedulers. These algorithms do not optimize TSP's runtime configurations (\eg execution time limits).
