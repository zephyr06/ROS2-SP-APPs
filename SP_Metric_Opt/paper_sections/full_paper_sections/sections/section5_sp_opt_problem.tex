\section{Safety-Performance Optimization Problem}
	...
	Tasks without a QoS budget have a constant performance function output of $1.0$, since their output quality does not depend on a configurable budget.
	For tasks with a QoS budget, the performance function maps the budget to a normalized quality score; we illustrate this with the TSP, an anytime algorithm whose output path shortens with a longer time limit.

	\begin{Example}
	For the TSP, we profiled running time limits from 50 ms upward and measured the resulting path length, averaging 10 runs per limit. The path lengths were normalized to $[0, 1.0]$, with the longest path scored $0.1$ and the shortest scored $1.0$; Table~\ref{table_tsp_perf} records the resulting performance function.
	\end{Example}
...
\end{itemize}
    ...
    First, since $\textbf{E}$ changes continuously and re-optimization intervals are short, only a few tasks' distributions change slowly between invocations, 
	so we can each warm-starts an incremental search from the last deployed configuration, and perform incremental optimization to focus on what's changed between scheduelr invocation.
    Second, the hard constraint~\eqref{eq_important_task_constraint} is enforced \emph{in-search}: the priority and QoS optimizers prune any candidate that violates an important task's deadline-miss threshold the moment it is formed, so the search never adopts an infeasible configuration.
	Next sections will explain with more details.
