    ...
    This motivates us to leverage the classical Audsley's algorithm~\cite{Audsley91optimalpriority} to optimize SP metrics due to optimal schedulability under certain conditions~\cite{Audsley91optimalpriority}, 
    and its $O(n^2)$ runtime complexity, where $n$ is the number of tasks in the task set. The challenges with directly applying Audsley's algorithm are that: it optimizes schedulability rather than the SP metric, so its assignments need not maximize SP, 
    and it assumes hard real-time constraints.
    
    To address these limitations, we modify Audsley's algorithm with a beam search that retains multiple partial priority assignments scored by accumulated SP-value loss rather than by schedulability alone, 
    preserving its $O(n^2)$ iterative structure.
    The modified algorithm works iteratively from the first iteration to the \ith{N} iteration where \ith{N} is the number of tasks in the task set $\boldsymbol{\tau}$.
...
Positions that would drive an important task over its threshold are skipped, so the incremental search, like the beam search, never moves to an infeasible assignment.
