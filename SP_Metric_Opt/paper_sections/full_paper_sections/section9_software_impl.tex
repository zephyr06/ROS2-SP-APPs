...
We designed and implemented a prototype system based on Linux, incorporating two types of periodic tasks: robotic application tasks and a scheduler task.
All tasks are scheduled using FIFO, Linux's real-time fixed-task priority scheduling algorithm.
The scheduler task is assigned the highest priority, and it optimizes the priority assignments and runtime configurations of robotic tasks based on the proposed algorithms.

The scheduler task executes once every $T$ time units.
During each execution, it:
\begin{itemize}
