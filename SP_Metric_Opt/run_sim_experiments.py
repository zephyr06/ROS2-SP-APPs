...
def main():
    ...
    # results_by_scheduler[scheduler] = {'sp_values': [], 'miss_rates': [], 'intervals': {}}
    ...
    for idx in range(args.n_tasksets):
        ...
        def run_single_simulation(scheduler, inst):
            sched_dir = os.path.join(taskset_dir, scheduler)
            sim_cmd = [
                sim_bin_path,
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            for scheduler in args.schedulers:
                for inst in range(args.n_inst):
                    sim_futures.append(executor.submit(run_single_simulation, scheduler, inst))
            # Wait for all simulations to complete
            concurrent.futures.wait(sim_futures)
        print("  --> All simulations completed. Starting analysis...")

        def analyze_single_instance(scheduler, inst):
            sched_dir = os.path.join(taskset_dir, scheduler)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            for scheduler in args.schedulers:
                for inst in range(args.n_inst):
                    analysis_futures.append(executor.submit(analyze_single_instance, scheduler, inst))
    ...
    sp_boxplot_data = [results_by_scheduler[s]['sp_values'] for s in args.schedulers]
    ax1.boxplot(sp_boxplot_data, labels=args.schedulers)
    ...
    print("\nAutomated experiment pipeline execution complete.")
