...
def main():
    ...
    #quit()
    if dbg:
        sim_rst_dir = r'TaskData\\taskset_cfg_10_1_gen_2'
        method = 'RM_FAST'
        interactive = True
        path_idx = 0
    else:
    ...
    ###############################################################################################
    # figure out number of processors
    g_num_processors = 0
    while True:
        task_config_file_path = os.path.join(sim_rst_dir, f'taskset_characteristics_i0_p{g_num_processors}.yaml')
    ...
    for pp in range(g_num_processors):
        ...
        while True: # get result of all simulation indexes for this processor
            ####################################################################################
            rst_file_path = os.path.join(sim_rst_dir, f'sim_res_{method}_{rst_idx}_p{pp}.txt')
            if not os.path.exists(rst_file_path):
                if rst_idx == 0:
                    print(f'no simulation result for {method}, processor={pp} found')
                    quit()
                else:
                    print(f'no more simulation result found for {method}, processor={pp}')
                    break
            ...
            for i in range(max_interval_idx + 1):
                ...
                if sp_value > -4.5:
                    a=1
                print(f'\n\n######## processor={pp}, Instance={rst_idx}, interval={i}: sp_value return for stdout: {sp_value}')
            ...
            #print(f'\n\n######## processor={pp}: Instance={rst_idx},sp_value_list={sp_value_list}')
            sp_value_file_path = os.path.join(out_dir, f"sp_value_{method}_{rst_idx}_p{pp}.txt")
            ...
            boxplot_data[pp].append((x_axis,sp_value_list))
            #print(f'######## processor={pp}: now boxplot_data len = {len(boxplot_data[pp])}')
            ...
            if interactive:
                ...
                plt.tight_layout()
                figure_path = os.path.join(out_dir, f"sp_value_plot_{method}_{rst_idx}_p{pp}.pdf")
    ...
    ############## now we have boxplot_data for each processor
    n_instance = len(boxplot_data[0])
    boxplot_data_sum = [] # for each instance, sum for all processors
    for i in range(n_instance):
        sp_value_list = boxplot_data[0][i][1]
        nn = len(sp_value_list)
        for pp in range(1,g_num_processors):
            for kk in range(nn):
                sp_value_list[kk] += boxplot_data[pp][i][1][kk]
        boxplot_data_sum.append( (boxplot_data[0][i][0],sp_value_list) )
        # print(f'instance={i}, boxplot_data_sum={boxplot_data_sum[i]}')

    # draw boxplot
    pdf_file_path = os.path.join(out_dir, f"box_plot_of_all_data_{method}.pdf")
