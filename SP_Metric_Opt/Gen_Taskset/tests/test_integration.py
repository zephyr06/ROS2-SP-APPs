...
def test_integration_pipeline():
    ...
    assert abs(total_weight - 10.0) < 1e-5
    ...
    # Soft task period must be >= MIN_PERIOD_WITH_PERFORMANCE_RECORDS (33)
    assert soft_task["period"] >= 33
    ...
    for task_idx, t in enumerate(char_data["tasks"]):
        ...
        for line in lines:
            ...
            # Assert coordinates are bounded by grid [-10, 10]
            assert -10 <= x <= 10
            assert -10 <= y <= 10
            
            # Assert execution time is within final bounds
            # period * FINAL_Et_OVER_PERIOD_RANGE [0.05, 0.9]
            assert period * 0.05 - 1e-4 <= et <= period * 0.9 + 1e-4
            assert et >= 1.0
            
            # Assert movement steps do not exceed ROBOT_STEP_SIZE (3.0)
            if prev_x is not None and prev_y is not None:
                # The change can only be at most ROBOT_STEP_SIZE in X or Y
                assert abs(x - prev_x) <= 3.0
                assert abs(y - prev_y) <= 3.0
