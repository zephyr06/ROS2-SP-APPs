...
int callTSP() {
    ...
    std::string PROJECT_PATH =
        std::string(std::string(SP_OPT_PROJECT_ROOT_DIR) +
                    "/applications/tsp_solver") +
        "/";
    ...
    if (Methods[2].compare(argv[3]) == 0) {
        cout << "Method:"
             << "Iterated Hill Climbing" << endl;

        filename1 =
            NAME + "_" + Methods[2] + "_" + argv[2] + "_" + argv[4] + ".sol";
        const char *name1 = filename1.c_str();
        filename2 =
            NAME + "_" + Methods[2] + "_" + argv[2] + "_" + argv[4] + ".trace";
    ...
    if (Methods[3].compare(argv[3]) == 0) {
        ...
        filename1 =
            NAME + "_" + Methods[3] + "_" + argv[2] + "_" + argv[4] + ".sol";
        const char *name1 = filename1.c_str();
        filename2 =
            NAME + "_" + Methods[3] + "_" + argv[2] + "_" + argv[4] + ".trace";
        const char *name2 = filename2.c_str();
        output_sol.open(name1);
        output_trace.open(name2);
        if (!output_sol.is_open() || !output_trace.is_open()) {
        ...
        path = SA(initial, distance, temp, 0.1 * dim, cooling_r, cutoff, dim,
                  seed);
    ...
    if (Methods[4].compare(argv[3]) == 0) {
        ...
        if (solution->time < cutoff) {
            ...
            cout << "Optimal cycle:";
            for (vector<int>::const_iterator it = solution->path->begin();
                 it != solution->path->end(); it++) {
                cout << *it + 1 << ",";
                output_sol << *it + 1 << ",";
            }
            cout << source + 1 << endl;
