...
| ~~**P2.2** Doc & memory hygiene~~ | [`finished_tasks/P2_2_doc_memory_hygiene/`](finished_tasks/P2_2_doc_memory_hygiene/) | **CLOSED 2026-07-12 as superseded / moot (NOT executed).** All substantive items overtaken by later resolutions: (a) `interval-sweep-stale-flags-bug` memory premise is stale in the *opposite* direction — `compare_optimizers.py` still accepts `--on_taskset_config_change` (`:334`) + `--run_root` (`:303`), so the described sweep-stage crash doesn't reproduce (and it was a CLI plumbing crash with **no algorithm-performance impact** anyway — the algorithm runs in the simulate stage); (b) §5 corrections predate the P1.1 resolution (`p25-ndiff-diff-semantics` re-derived "2-vs-8" as false-positives, NOT UNRESOLVED; P0.5 then resolved the gate `ndiff` 5→0) — both §5 asks moot; (c) `issues.md` deleted in `66c96c14` (2026-07-06) — resolved-by-deletion; (d) `trial_and_error` already DONE (`88af2c54`+`fa0b857f`) + P0.1 landed. No code/doc/memory edited — only the closure recorded. Memory file itself left untouched (out of scope). See `finished_tasks/summary.md`. |
...
| `PriorityPartialPath` → pointers | [`P1_1_efficiency_optimizations/`](active_tasks/P1_1_efficiency_optimizations/) §A | Pure perf; runtime figures adequate without it. Revisit only if P1.1 finds ET is a paper blocker. |
| Reuse one challenger across `EvaluateTimeLimitConfig_ScratchOrIncre` calls | [`P1_1_efficiency_optimizations/`](active_tasks/P1_1_efficiency_optimizations/) §3 | Moved from P0.5 Phase-5 issue 5h on 2026-07-10. Perf only. The rebuild-from-champion design (P0.5 5b) was chosen over the persistent challenger — drift risk to the diff baseline. Pick up only if profiling shows the rebuild is a blocker. |
...
anytime until it needs a code change) → ~~P2.2~~ CLOSED 2026-07-12 (all items
superseded/moot — `interval-sweep` memory premise stale, §5 pre-P1.1-resolution,
`issues.md` deleted in `66c96c14`; NOT executed) → P2.3 (`approx_equal`
