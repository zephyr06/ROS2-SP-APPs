...
- **P2.13 (important-task DDL vs SP metric — study) FILED 2026-07-25.** Triggered
  by the user's finding: in the N=10 `INCR_Reopt_1`/`_10` run,
  `Important_Miss_Rate` swings (0.48→0.567) while `Mean_SP_Metric` stays flat
  (~0.578). 4-reason answer found this session: (1) SP is analytic
  RTA×weight (`SP_Metric.cpp:12-15`), `Important_Miss_Rate` is an empirical
  job-fraction on 1 task (`utils.py:176-216`) — different signals; (2) `SP_Func`
  saturates (`SP_Metric.h:31-41`: penalty plunges only near v→1.0; for th=0.2,
  v=0.48→0.989 vs v=0.567→0.981 — a 0.087 miss gap moves SP by ~0.008); (3)
  weight dilution (1 "important" task @ ~0.1333 of Σ=1.0; the 9 low-miss tasks
  dominate); (4) the important task misses 100% in BOTH arms → SP≈0 for it in
  both → no cross-arm SP difference from the swing task. Three facets to
  characterize (NOT fix here): metric (`SP_Func` saturation), generator (P1.8 H1d:
  DDL independent of ET → infeasibly tight DDL?), selection (1 important task +
  5-way weight tie → `Important_Miss_Rate` may be label-noise). **Decisive next
  step = feasibility check (DDL vs WCET) on the existing run, NO new sim:** if
  DDL<WCET → flat SP is *correct* (unschedulable task scores ~0 in both arms),
  owner = P1.8; if DDL≥WCET yet high empirical miss → metric under-penalizes,
  file a new task for the `SP_Func` shape (D3). Cross-links P2.6 (sim-RT SP, D4
  deferred), P1.8 (generator defect). Analysis study only — no implementation
  until greenlit; no `git commit`; no YAML/gate/`SP_Func` change in this task.
  Full record: `agents/active_tasks/P2_13_important_task_ddl_vs_sp_metric/`.
