...
int main(int argc, char *argv[]) {
    ...
    program.add_argument("--output_folder")
        .default_value(std::string("TaskData/taskset_cfg_1_gen_1"))
        .help(
    ...
    while (1) {
        ...
        std::cout << "\n\n\n\n######## simulation for instance: " << path_idx << " ..." << std::endl;
        ...
        for (int i = 0; i < num_tasks; i++) {
            string file_path1 = input_folder + 
                "/path_Et_task_" + std::to_string(i) + "_" + std::to_string(path_idx) +".txt";
            std::ifstream infile(file_path1);
            if (!infile.is_open()) {
                if (path_idx==0) {
                    std::cerr << "Failed to open the file: " << file_path1 << std::endl;
                } else {
                    std::cout<<"\n\n\n\n######## "<<(path_idx)<<" instances are simulated"<<std::endl;
                }
                return 1;
            }
            std::string line;
        ...
        string output_file_path = output_folder + "/sim_res_" + sched_policy + "_" + std::to_string(path_idx) + ".txt";
        std::ofstream *output_file = nullptr;
        output_file = new std::ofstream(output_file_path);
        // Check if the file was opened successfully
        if (!output_file->is_open()) {
        ...
        string log_file_path = output_folder + "/sim_log_" + sched_policy + "_" + std::to_string(path_idx) + ".txt";
        ...
        if (sim_api == "SimulatedCSP_SingleCore") {
            schedule_actual = SimulatedCSP_SingleCore(dag_tasks, tasks_info, sp_parameters, 0, 
                                                        ...
                                                        output_job_not_executed,log_file); 
        ...
        if (GlobalVariables::debugMode & DBG_PRT_MSK_MAIN) {
            std::cout << "####main: "<< sim_api << " DONE for instance " << path_idx << std::endl;        
        }
