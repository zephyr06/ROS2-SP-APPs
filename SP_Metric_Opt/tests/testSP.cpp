...
TEST_F(TaskSetForTest_2tasks, SP_Calculation) {
  ...
  double sp_norm = 1 + (log(1 + 0.5 - 0.0012) - (-0.01 * exp(10 * abs(0.5)))) /
...
public:
  void SetUp() override {
    ...
    std::string path = GlobalVariables::PROJECT_PATH +
                       "TaskData/test_AnalyzeSP/" + file_name + ".yaml";
...
TEST_F(TaskSetForTest_robotics_v18, Task2priority_value) {
  auto task2priority =
      Task2priority_value(dag_tasks.tasks, {0, 1, 2, 3}); // TSP, MPC, RRT, SLAM
  ...
  task2priority =
      Task2priority_value(dag_tasks.tasks, {3, 2, 1, 0}); // SLAM, RRT, MPC, TSP
...
TEST_F(TaskSetForTest_robotics_v20, ObtainSPFromRTAFiles) {
  string slam_path = GlobalVariables::PROJECT_PATH +
...
TEST_F(Test_testAnalyzeSP, ObtainSPFromRTAFiles2) {
  string file_path_ref = GlobalVariables::PROJECT_PATH +
                         "TaskData/test_AnalyzeSP/" +
                         "taskset_characteristics_0.yaml";
...
TEST_F(Test_testAnalyzeSP, GetAvgTaskPerfTerm) {
  ...
  for (uint i = 8; i < dag_tasks.tasks.size(); i++) {
    std::string rst_path =
    ...
    if (dag_tasks.tasks[i].timePerformancePairs.size() > 0) {
      std::string ext_path =
          data_dir + dag_tasks.tasks[i].name + "_execution_time.txt";
      ...
      double weight =
          GetAvgTaskPerfTerm(ext_path, dag_tasks.tasks[i].timePerformancePairs);
      ...
      std::cout << "ERROR! task_id=" << i << ", no timePerformancePairs"
                << std::endl;
    }
  }
  ...
  for (uint i = 0; i < dag_tasks.tasks.size(); i++) {
    ...
    printf("%d, task%d: deadline=%.0f, weight=%f, threshold=%f\n", i, task_id,
           dag_tasks.tasks[i].deadline, weight, threshold);
    printf("response_time dist ...");
    node_rts_dists[i].print();

    double ddl_miss_chance =
        GetDDL_MissProbability(node_rts_dists[i], dag_tasks.tasks[i].deadline);
